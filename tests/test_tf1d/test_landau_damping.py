    ...
    with mlflow.start_run(run_id=run_id, log_system_metrics=True) as mlflow_run:
        kx = (
            np.fft.fftfreq(
                ...
                d=mod_defaults["save"]["x"]["ax"][2]
                - mod_defaults["save"]["x"]["ax"][1],
            )
            * 2.0
            * np.pi
        )
        ...
        efs = jnp.real(
            jnp.fft.ifft(
                1j
                * one_over_kx[None, :]
                * jnp.fft.fft(1 - result.ys["x"]["electron"]["n"][:, :])
            )
        )
        ek1 = (
            2.0 / mod_defaults["grid"]["nx"] * np.abs(np.fft.fft(efs, axis=1)[:, 1])
        ) ** 2.0
        frslc = slice(-100, -50)
        measured_damping_rate = np.mean(
            np.gradient(ek1[frslc], (result.ts[1] - result.ts[0])) / ek1[frslc]
        )
        print(
        ...
        np.testing.assert_almost_equal(
            measured_damping_rate, 2 * actual_damping_rate, decimal=2
        )
