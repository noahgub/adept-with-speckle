...
class ergoExo:
    ...
    def val_and_grad(
    ...
    ) -> Tuple[float, Dict, Dict, Tuple[Solution, Dict, str]]:
        """
        ...
        with mlflow.start_run(
            ...
            model_cfg = static_laser_driver_object.model_cfg
            metrics_to_log = {}

            for param_name, param_config in model_cfg.items():
                if not param_config.get('learned'):
                    continue
                unbdd_value = getattr(laser_driver_object, param_name)
                if unbdd_value is None:
                    continue
                # Calculate bounded value only if bounds exist
                bdd_value = None
                if 'a' in param_config and 'b' in param_config:
                    bdd_value = param_config['a'] + param_config['b'] * jnp.tanh(unbdd_value)

                # Helper function to log metrics for a single value
                def log_single_metric(name, unbounded, bounded=None):
                    metrics_to_log[f"Unbounded {name}"] = float(unbounded)
                    if bounded is not None:
                        metrics_to_log[f"Bounded {name}"] = float(bounded)

                # Handle both arrays and scalars
                if jnp.shape(unbdd_value) == ():
                    log_single_metric(param_name, unbdd_value, bdd_value)
                elif jnp.shape(unbdd_value) == (2,):
                    for i in range(2):
                        bound_val = bdd_value[i] if bdd_value is not None else None
                        log_single_metric(f"{param_name}_{i}", unbdd_value[i], bound_val)
            mlflow.log_metrics(metrics_to_log)

            t0 = time.time()
            with tempfile.TemporaryDirectory(dir=self.base_tempdir) as td:
                post_processing_output = self.adept_module.post_process(run_output, td, laser_driver_object)
                if export:
                    robust_log_artifacts(td)  # logs the temporary directory to mlflow
                if "metrics" in post_processing_output:
                    mlflow.log_metrics(post_processing_output["metrics"])
            ...
            return val, grad, metrics_to_log, (run_output, post_processing_output, self.mlflow_run_id)
