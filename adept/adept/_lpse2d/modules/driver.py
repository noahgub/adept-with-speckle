...
class SpeckledDriver(UniformDriver):
    ...
    def __init__(self, cfg: Dict):
        ...
        self.ssd_transverse_bandwidth_distribution = jnp.array(driver_cfg["lasy_params"]["ssd_transverse_bandwidth_distribution"])
        self.phase_plate = jax.random.uniform(PRNGKey(0), shape=(self.n_beamlets_x, self.n_beamlets_y), minval=0, maxval=2*jnp.pi)
        # Unbound for optimization
        if self.model_cfg["relative_laser_bandwidth"]["learned"]:
            self.relative_laser_bandwidth = self.bound_param(self.relative_laser_bandwidth, "relative_laser_bandwidth", False)
        if self.model_cfg["ssd_phase_modulation_amplitude"]["learned"]:
            self.ssd_phase_modulation_amplitude = self.bound_param(self.ssd_phase_modulation_amplitude, "ssd_phase_modulation_amplitude", False)
        if self.model_cfg["ssd_number_color_cycles"]["learned"]:
            self.ssd_number_color_cycles = self.bound_param(self.ssd_number_color_cycles, "ssd_number_color_cycles", False)
        if self.model_cfg["ssd_transverse_bandwidth_distribution"]["learned"]:
            self.ssd_transverse_bandwidth_distribution = self.bound_param(self.ssd_transverse_bandwidth_distribution, "ssd_transverse_bandwidth_distribution", False)
        if self.model_cfg["phase_plate"]["learned"]:
            self.phase_plate = self.bound_param(self.phase_plate, "phase_plate", False)
    ...
    def get_partition_spec(self):
        ...
        if self.model_cfg["ssd_phase_modulation_amplitude"]["learned"]:
            filter_spec = eqx.tree_at(lambda tree: tree.ssd_phase_modulation_amplitude, filter_spec, replace=True)
        if self.model_cfg["ssd_number_color_cycles"]["learned"]:
            filter_spec = eqx.tree_at(lambda tree: tree.ssd_number_color_cycles, filter_spec, replace=True)
        if self.model_cfg["ssd_transverse_bandwidth_distribution"]["learned"]:
            filter_spec = eqx.tree_at(lambda tree: tree.ssd_transverse_bandwidth_distribution, filter_spec, replace=True)
        if self.model_cfg["phase_plate"]["learned"]:
            filter_spec = eqx.tree_at(lambda tree: tree.phase_plate, filter_spec, replace=True)
    ...
    def __call__(self, state: Dict, args: Dict) -> tuple:
        ...
        bdd_phase_plate = self.phase_plate

        if self.model_cfg["relative_laser_bandwidth"]["learned"]:
            bdd_relative_laser_bandwidth = self.bound_param(self.relative_laser_bandwidth, "relative_laser_bandwidth", True)
        if self.model_cfg["ssd_phase_modulation_amplitude"]["learned"]:
            bdd_ssd_phase_modulation_amplitude = self.bound_param(self.ssd_phase_modulation_amplitude, "ssd_phase_modulation_amplitude", True)
        if self.model_cfg["ssd_number_color_cycles"]["learned"]:
            bdd_ssd_number_color_cycles = self.bound_param(self.ssd_number_color_cycles, "ssd_number_color_cycles", True)
        if self.model_cfg["ssd_transverse_bandwidth_distribution"]["learned"]:
            bdd_ssd_transverse_bandwidth_distribution = self.bound_param(self.ssd_transverse_bandwidth_distribution, "ssd_transverse_bandwidth_distribution", True)
        if self.model_cfg["phase_plate"]["learned"]:
            bdd_phase_plate = self.bound_param(self.phase_plate, "phase_plate", True)
