...
class IMPACT:
    ...
    def __init__(self, cfg):
        ...
        self.nuee_coeff = (
            4.0 * np.pi / 3.0 * c_kpre * cfg["units"]["derived"]["logLambda_ee"]
        )
    ...
    def calc_Ij(self, this_f, j=0):
        return (
            ...
            * jnp.pi
            / self.v**j
            * jnp.cumsum(this_f * self.v ** (2.0 + j), axis=1)
            * self.dv
        )
    ...
    def calc_C1(self, f1, f0, I00, I20, Jm10, I31, Jm21, I11):
        ...
        return self.nuei_coeff * (
            0 * term1 + 0 * term2 + term3 + 0 * term4 + 0 * term5 + 0 * term6
        )
    ...
    def get_step(self, prev_f0, prev_f1):
        ...
        def _step_(this_y):
            ...
            prev_f0_approx = f0 + self.dt * (
                ...
                / 3.0
                / self.v[None, :] ** 2.0
                * self.ddv_f1(self.v[None, :] ** 2.0 * (e[:, None] * f1))
                - Cee0
            )
            C_f1 = self.calc_C1(f1, f0, I00, I20, Jm10, I31, Jm21, I11)
            prev_f1_approx = f1 + self.dt * (
                self.v[None, :] * self.ddx(f0) - e[:, None] * self.ddv(f0) - C_f1
            )
            ...
            j = (
                ...
                * jnp.pi
                / 3.0
                * jnp.sum(f1 * self.v[None, :] ** 3.0, axis=1)
                * self.dv
            )
    ...
    def __call__(self, t, y, args) -> Dict:
        ...
        operator = lx.FunctionLinearOperator(
            self.get_step(f0, f1), input_structure={"f0": f0, "f1": f1, "e": e}
        )
        rhs = {"f0": f0, "f1": f1, "e": e}
        solver = lx.BiCGStab(
            rtol=1e-3,
        ...
        sol = lx.linear_solve(
            operator, rhs, solver=solver, options={"y0": {"f0": f0, "f1": f1, "e": e}}
        )
