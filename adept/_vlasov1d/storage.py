    ...
    if any(x in ["x", "kx"] for x in cfg["save"][prefix].keys()):
        ...
        for k, v in fields.items():
            das[f"{prefix}-{k}"] = xr.DataArray(
                v[:, 1:-1] if k in ["a", "prev_a"] else v,
                coords=(("t", this_t), ("x", cfg["grid"]["x"])),
            )
...
def store_diags(cfg: Dict, this_t: Dict, td: str, ys: Dict) -> xr.Dataset:
    ...
    diags_store = xr.Dataset(
        {
            spc: xr.DataArray(ys[spc], coords=axes)
            for spc in ["diag-vlasov-dfdt", "diag-fp-dfdt"]
        }
    )
    ...
    if {"t"} == set(cfg["save"]["fields"].keys()):
        ...
        def fields_save_func(t, y, args):
            temp = {
                ...
                "v": _calc_moment_(y["electron"] * cfg["grid"]["v"][None, :]),
            }
...
def get_save_quantities(cfg: Dict) -> Dict:
    ...
    :return:
    """
    for save_type, save_config in cfg[
        "save"
    ].items():  # this can be fields or electron or diags?
        for (
            dim_key,
            dim_config,
        ) in save_config.items():  # this can be t, x, y, kx, ky (eventually)
            if dim_key == "x":
                dx = (
                    dim_config[f"{dim_key}max"] - dim_config[f"{dim_key}min"]
