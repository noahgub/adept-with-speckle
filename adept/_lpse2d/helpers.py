...
def get_solver_quantities(cfg: Dict) -> Dict:
    ...
    cfg_grid["zero_mask"] = (
        np.where(
            np.sqrt(cfg_grid["kx"][:, None] ** 2 + cfg_grid["ky"][None, :] ** 2) == 0,
            0,
            1,
        )
...
def plot_fields(fields, td):
    ...
    for k, v in fields.items():
        ...
        x_slice_dir = os.path.join(fld_dir, "slice-along-x")
        os.makedirs(x_slice_dir)

        np.log10(np.abs(v[tslice, :, ymidpt])).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(x_slice_dir, f"log-{k}.png"))
        plt.close()

        np.abs(v[tslice, :, ymidpt]).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(x_slice_dir, f"{k}.png"))
        plt.close()

        np.real(v[tslice, :, ymidpt]).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(x_slice_dir, f"real-{k}.png"))
        plt.close()

        np.log10(np.abs(v[:, :, ymidpt])).plot()
        plt.savefig(os.path.join(x_slice_dir, f"spacetime-log-{k}.png"))
        plt.close()

        np.abs(v[:, :, ymidpt]).plot()
        plt.savefig(os.path.join(x_slice_dir, f"spacetime-{k}.png"))
        plt.close()

        np.real(v[:, :, ymidpt]).plot()
        plt.savefig(os.path.join(x_slice_dir, f"spacetime-real-{k}.png"))
        plt.close()

        xmidpt = int(fields.coords["x (um)"].data.size // 2)
        y_slice_dir = os.path.join(fld_dir, "slice-along-y")
        os.makedirs(y_slice_dir)

        np.log10(np.abs(v[tslice, xmidpt, :])).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(y_slice_dir, f"log-{k}.png"))
        plt.close()

        np.abs(v[tslice, xmidpt, :]).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(y_slice_dir, f"{k}.png"))
        plt.close()

        np.real(v[tslice, xmidpt, :]).plot(col="t (ps)", col_wrap=4)
        plt.savefig(os.path.join(y_slice_dir, f"real-{k}.png"))
        plt.close()

        np.log10(np.abs(v[:, xmidpt, :])).plot()
        plt.savefig(os.path.join(y_slice_dir, f"spacetime-log-{k}.png"))
        plt.close()

        np.abs(v[:, xmidpt, :]).plot()
        plt.savefig(os.path.join(y_slice_dir, f"spacetime-{k}.png"))
        plt.close()

        np.real(v[:, xmidpt, :]).plot()
        plt.savefig(os.path.join(y_slice_dir, f"spacetime-real-{k}.png"))
        plt.close()
...
def make_field_xarrays(cfg, this_t, state, td):
    ...
    phi_k = xr.DataArray(
        ...
        coords=(
            tax_tuple,
            ("kx ($kc\omega_0^{-1}$)", shift_kx),
            ("ky ($kc\omega_0^{-1}$)", shift_ky),
        ),
    )
    ex_k = xr.DataArray(
    ...
    kfields.to_netcdf(
        os.path.join(td, "binary", "k-fields.xr"),
        engine="h5netcdf",
        invalid_netcdf=True,
    )
    fields.to_netcdf(
...
def get_save_quantities(cfg: Dict) -> Dict:
    ...
    if "x" in cfg["save"]["fields"]:
        ...
        xq, yq = jnp.meshgrid(
            cfg["save"]["fields"]["x"]["ax"],
            cfg["save"]["fields"]["y"]["ax"],
            indexing="ij",
        )
