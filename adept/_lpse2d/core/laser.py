...
class Light:
    ...
    def lasy_update(self, t, y, light_wave):
        ...
        E0y = laser_profile.evaluate(x_with_expanded_dims, y_with_expanded_dims, t_with_expanded_dims)[
            ..., 0
        ...
        E0_static = (1 + 0j - wpe**2.0 / (self.w0 * (1 + delta_omega)) ** 2) ** -0.25 * self.E0_source
        ...
        E0y *= 1e-12 * (1e-4) ** 0.5  # (g^0.5 / cm^0.5 s) --> (g^0.5 / um^0.5 ps), LPSE units
        # Filter out high frequency modes
        E0y_fft = jnp.fft.fft(E0y, axis=1)  # FFT along the second axis since E0y.shape = (1,ny)
